...
from Prima_training_and_evaluation import (RachelDataset, CLIP, ViT,
                                           GPTWrapper, HierViT,
                                           SerieTransformerEncoder, MrDataset)
...
class TestModelLoading(unittest.TestCase):
    ...
    def _create_dummy_models(cls):
        ...
        vqvae = torch.nn.Sequential(torch.nn.Conv3d(1, 32, 3), torch.nn.ReLU(),
        ...
        head = torch.nn.Sequential(torch.nn.Linear(256, 128), torch.nn.ReLU(),
    ...
    def test_vqvae_loading(self):
        ...
        self.assertEqual(str(vqvae.device),
    ...
    def test_prima_model_loading(self):
        ...
        self.assertEqual(str(prima.device),
                         f'cuda:0' if torch.cuda.is_available() else 'cpu')
        ...
        dummy_input = {
            'visual':
            torch.randn(1, 10,
                        1024),  # batch_size, sequence_length, embedding_dim
    ...
    def test_dataset_loading(self):
        ...
        try:
            dataset = MrDataset(**dataset_config)
            self.assertIsInstance(dataset, MrDataset)
