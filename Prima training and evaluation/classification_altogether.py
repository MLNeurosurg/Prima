...
from dataset import collatevisualhash,collateembhash,RachelDataset

from utils import getbestthresh
import numpy as np
device='cuda:0'
class ClassificationTask:
    def __init__(self,dataset,visualmodel,poslist,patchify,protobatchsize=12,vallist = None,retpool=True):
        ...
        loader = torch.utils.data.DataLoader(dataset,batch_size=protobatchsize,shuffle=False,num_workers=12,collate_fn=collatevisualhash(patchify,device))
        with torch.no_grad():
            for d in tqdm(loader):
                h = d['hash']
                with torch.amp.autocast(device_type='cuda',dtype=torch.float16):
                    embeds = visualmodel(xdict=d,retpool=retpool)
                labels = [[(1 if (hh in pl) else 0) for pl in poslist] for hh in h]
        ...
        for i in range(len(self.allembeds)):
            ...
            self.allembeds[i] = (t,[(1 if h in pl else 0) for pl in poslist],h)
        ...
        self.criterionweighted = torch.nn.BCEWithLogitsLoss(pos_weight=torch.FloatTensor(ratios).to(device))
        self.criterion = torch.nn.BCEWithLogitsLoss()
        self.softmax = torch.nn.Softmax(dim=1)
        self.visembedlen = len(self.allembeds[0][0])

    def trainandval(self,model,optimizer,batch_size=200):
        trainloader = torch.utils.data.DataLoader(self.trainembedsbalanced,batch_size=batch_size,shuffle=True,collate_fn=collateembhash)
        valloader = torch.utils.data.DataLoader(self.valembeds,batch_size=batch_size,collate_fn=collateembhash)
        totalloss = 0.0
        totals = 0
        ...
        with torch.no_grad():
            for t,l,h in valloader:
                ...
                for i in range(len(l)):
                    ...
                    correctpos.append(torch.logical_and(pred == 1,l[i]==1).long())
                    correctneg.append(torch.logical_and(pred == 0,l[i]==0).long())
        ...
        for j in range(classnum):
            ...
            bestthreshs.append(getbestthresh(gts,values)) # get the best cutoff threshold for binary classification

        return trainloss,valloss,valacc,valauc,correctposlist,correctneglist,(fullpred > 0).astype(np.int32),bestthreshs
...
def loadvismodel(path,devices):
    ...
    model = torch.nn.DataParallel(m.module.visual_model,device_ids=devices).to(device)
    model.module.patdis=False
    return model,m.module.patchifier.to('cpu')

# main training loop for a specific checkpoint
def train(protodataset,vismodelpath,poslist,vallist=None,epochs=45, classnum=1,devices=[0],savesite = '.',cnames = []):
    vismodel,patchify = loadvismodel(vismodelpath,devices)
    cls = ClassificationTask(protodataset,vismodel,poslist,patchify,vallist = vallist)
    ...
    newmodel = torch.nn.Sequential(torch.nn.Linear(cls.visembedlen,4000),torch.nn.ReLU(),torch.nn.Linear(4000,1000),torch.nn.ReLU(),torch.nn.Linear(1000,classnum))
    newmodel = torch.nn.DataParallel(newmodel,device_ids=config['train']['devices']).to(device)
    optim = torch.optim.RMSprop(newmodel.parameters(),lr=0.00001)
    for e in range(epochs):
        tloss,vloss,vacc,vauc,cpos,cneg,fpred,threshs = cls.trainandval(newmodel,optim)
        ...
        for i in range(classnum):
            if vauc[i] > bestvauc[i]:
                ...
                torch.save(newmodel.module,savesite+'/bestauc_'+cnames[i]+'.pt')
...
if __name__=='__main__':
    ...
    protodataset = thedataset(datajson=config['data']['datajson'],
            ...
            novisualaug = True,
            nosplit = config['data']['nosplit'] if 'nosplit' in config['data'] else False
            )
    nums = config['train']['nums']
    paths = [(config['train']['ckptsavedir']+'/'+str(num)+'.pt',num) for num in nums]
    poslist = [open(pl).read().split('\n') for pl in config['cset']['txtnames']]
    ...
    for path,num in paths:
        bestvauc,bestvacc, bestaucfpred, bestaccfpred, cls = train(protodataset,path,poslist,vallist = vallists,epochs = config['train']['epochs'],classnum = config['cset']['classnum'], savesite=mysavesite+'/'+config['cset']['markdate'],cnames=config['cset']['names'],devices = config['train']['devices'])
