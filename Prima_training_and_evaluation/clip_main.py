...
import argparse
from dataset import collate, MrDataset, SubDataset
...
def main(config):
    ...
    train_dataset = thedataset(
        ...
        prospectivedatalist=None if 'datalistcsv' not in config['data'] else
        config['data']['datalistcsv'])
    ...
    if 'clip_ckpt' not in config['model']:
        ...
        model = torch.nn.DataParallel(model,
                                      device_ids=config['train']['devices'])
    else:
        print('loading existing clip model')
        model = torch.load(config['model']['clip_ckpt'])
        if 'forcetextckpt' in config['model']['text'] and config['model'][
                'text']['forcetextckpt']:
            model.module.replacetextmodel(config['model']['text']['ckpt_path'],
                                          config['model']['feature_dim'])
        patching = copy.deepcopy(model.module.patchifier.to('cpu'))
        model.module.patchifier.to(device)
        model.module.visual_model.patdis = 'patient_series_discrimination' in config[
            'train']
        if model.module.visual_model.patdis is not None and model.module.visual_model.patdis:
            if not isinstance(model.module.patdistemperature,
                              torch.nn.Parameter):
                model.module.patdistemperature[0] = config['train'][
                    'patdis_init_temperature']
                model.module.patdistemperature = torch.nn.Parameter(
                    model.module.patdistemperature).to(device)
    ...
    if ('early_stop_step' in config['train']) and (
            len(train_dataset) > config['train']['early_stop_step'] *
            config['data']['batch_size'] * cumbatchnum):
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        collate_fn=collate(config['data']['text_max_tokens'], patching, 'cpu',
                           0),
        shuffle=True,
    ...
    for e in range(config['train']['epochs']):
        ...
        if 'early_stop_step' in config['train'] and isinstance(
                train_sub_dataset, SubDataset):
        ...
        try:
            ...
            for batchnum, batch in enumerate(tqdm(train_loader)):
                ...
                with torch.amp.autocast(device_type='cuda',
                    ...
                    if patdis is not None:
                        ...
                        patdisloss, patdisagg = patdis_objective(
                            patdisres, mlist, model.module.patdistemperature)
                ...
                if len(out1) == config['data']['batch_size'] * cumbatchnum:
                    ...
                    tolog = {
                        ...
                        losstemp.item(),
                        'temperature':
                        model.module.temperature.item(),
                        'patdisloss':
                        0 if patdis is None else patdisloss.item(),
                        'patdistemperature':
                        0 if patdis is None else
                        model.module.patdistemperature.item(),
                        'patdisagg':
                        0 if patdis is None else patdisagg.item()
                    }
            ...
            if 'early_stop_step' in config['train']:
                train_sub_dataset = SubDataset(
                    train_dataset, config['train']['early_stop_step'] *
            ...
            train_loader = torch.utils.data.DataLoader(
                train_sub_dataset,
                batch_size=config['data']['batch_size'],
                collate_fn=collate(config['data']['text_max_tokens'], patching,
                                   'cpu', 0),
                shuffle=True,
                num_workers=config['train']['num_train_loader_workers'],
                timeout=3600)
        ...
        with torch.no_grad():
            ...
            try:
                ...
                val_loader = torch.utils.data.DataLoader(
                    val_dataset,
                    batch_size=config['data']['batch_size'],
                    collate_fn=collate(config['data']['text_max_tokens'],
                                       patching, 'cpu', 0),
                    shuffle=False,
                    num_workers=config['train']['num_val_loader_workers'],
                    timeout=1200)
            ...
            if patdis is not None:
                ...
                patdisloss, patdisagg = patdis_objective(
                    patdisscores, mress, model.module.patdistemperature)
