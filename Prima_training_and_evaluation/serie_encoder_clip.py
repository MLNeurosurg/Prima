...
def main(config):
    ...
    train_loader = torch.utils.data.DataLoader(train_sub_dataset,batch_size=config['data']['batch_size'],collate_fn = collateserienameclip(patching),shuffle=True,num_workers=config['train']['num_train_loader_workers'],timeout=7200)
    val_loader = torch.utils.data.DataLoader(val_dataset,batch_size=config['data']['batch_size'],collate_fn = collateserienameclip(patching),shuffle=False,num_workers=config['train']['num_val_loader_workers'],timeout=1200)
    ...
    for e in range(config['train']['epochs']):
        ...
        try:
            ...
            traceback.print_exc()
            if 'early_stop_step' in config['train']:
                train_sub_dataset = SubDataset(train_dataset,config['train']['early_stop_step']) # a subdataset if we do early stop
            else:
                train_sub_dataset = train_dataset
            train_loader = torch.utils.data.DataLoader(train_sub_dataset,batch_size=config['data']['batch_size'],collate_fn = collateserienameclip(patching),shuffle=True,num_workers=config['train']['num_train_loader_workers'],timeout=7200)
            continue
...
def retrievaleval(embs1,embs2, k):
    ...
    tcorrect = torch.sum(textretrievals == torch.arange(len(embs1)).to(embs1.device))
    icorrect = torch.sum(imageretrievals == torch.arange(len(embs1)).to(embs1.device))
    return tcorrect/len(embs1), icorrect/len(embs1), textretrievals, imageretrievals
