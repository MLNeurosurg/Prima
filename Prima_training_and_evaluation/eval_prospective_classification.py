...
if __name__=='__main__':
    ...
    model = torch.nn.DataParallel(model.module.visual_model,device_ids=[0]).to('cuda:0')
    ...
    # record results
    preddict = {name:{m:[] for m,_ in evaldict[name][1]} for name in evaldict}
    gts = {name:[] for name in evaldict}
    model.module.patdis = False
    dataloader = torch.utils.data.DataLoader(dataset,batch_size=config['data']['batch_size'],collate_fn = collatevisualhash(p,'cpu'),shuffle=False,num_workers=7,timeout=7200)
    with torch.no_grad():
        ...
        for batch in tqdm(dataloader):
            ...
            for name in evaldict:
                ...
                for m,i in modellist:
                    #with torch.amp.autocast(device_type='cuda',dtype=torch.float16):
                    pred = heads[m](embeds.to('cpu').float())[:,i] - heads[m].thresh
                    preddict[name][m].append(pred)

        # run metrics
        for name in preddict:
            for m in preddict[name]:
                preddict[name][m] = torch.cat(preddict[name][m],dim=0).detach().cpu().tolist()
                bas,se,sp = evalacc(gts[name],preddict[name][m],0)
                print(name+' model:  balanced accuracy score: '+str(bas) + '    \t\t'+m)   
