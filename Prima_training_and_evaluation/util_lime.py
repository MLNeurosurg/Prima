...
class Lime_Explainer:
    ...
    def explain_instance(self,
        ...
        masks = (randomstate.randint(0, fracs + 1,
                                     lentokens * samples).reshape(
                                         samples,
                                         lentokens).astype(np.float64))
        masks /= float(fracs)
        masks[0] = 1
        ...
        for i in tqdm(range(samples)):
            ...
            newdata['visual'][series_ord] = newdata['visual'][series_ord][
                indices]
            newdata['coordinates'][series_ord] = newdata['coordinates'][
                series_ord][indices]
            llabels[i, 0] = classfn([newdata])

        ret = self.base.explain_instance_with_data(masks,
                                                   llabels,
                                                   distances,
                                                   0,
                                                   lentokens,
